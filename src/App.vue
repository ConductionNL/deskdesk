...
<template>
	<NcContent app-name="deskdesk">
		<template v-if="storesReady && !hasOpenRegisters">
			<NcAppContent class="open-register-missing">
				<NcEmptyContent
					:name="t('deskdesk', 'OpenRegister is required')"
					:description="t('deskdesk', 'This app needs OpenRegister to store and manage data. please install OpenRegister from the app store to get started.')">
					<template #icon>
						<img :src="appIcon"
							alt=""
							width="64"
					...
					<template #action>
						<NcButton
							...
							type="primary"
							:href="appStoreUrl">
							{{ t('deskdesk', 'Install OpenRegister') }}
						</NcButton>
					</template>
				</NcEmptyContent>
			</NcAppContent>
...
export default {
	...
	computed: {
		...
		appIcon() {
			return imagePath('deskdesk', 'app-dark.svg')
		},
		appStoreUrl() {
