...
<template>
	<CnDetailPage
		:title="itemData.title || t('deskdesk', 'Item')"
		:subtitle="itemData.identifier || ''"
		:back-route="{ name: 'Items' }"
		:back-label="t('deskdesk', 'Back to list')"
		:loading="loading"
		:sidebar="!isNew && !loading"
		object-type="deskdesk_item"
		:object-id="itemId"
		...
		<template #actions>
			<NcButton
				...
				:disabled="saving"
				@click="save">
				{{ saving ? t('deskdesk', 'Saving…') : t('deskdesk', 'Save') }}
			</NcButton>
			<NcButton
				v-if="!isNew && !loading"
				type="error"
		...
		<NcDialog
			...
			:name="t('deskdesk', 'Delete item')"
			:message="t('deskdesk', 'Are you sure you want to delete this item?')"
...
export default {
	...
	computed: {
		...
		sidebarProps() {
			...
			return {
				title: this.itemData.title || this.t('deskdesk', 'Item'),
		...
		deleteDialogButtons() {
			return [
				{ label: this.t('deskdesk', 'Cancel'), type: 'secondary' },
				{ label: this.t('deskdesk', 'Delete'), type: 'error' },
			]
		},
	},
