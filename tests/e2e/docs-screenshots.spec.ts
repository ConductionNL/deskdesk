...
// `/apps/deskdesk/` so a selector miss on one doesn't cascade.
...
test.describe('docs: admin track', () => {
	test.beforeEach(async ({ page }) => {
		await page.goto('/settings/admin/deskdesk')
		await page.waitForLoadState('networkidle')
	})

	test('UN floors-zones', async ({ page }) => {
		// docs/tutorials/admin/01-floors-zones.md
		// TODO: see /journeydoc-add-story
		await shoot(page, 'admin', '01-floors-zones.png')
	})

	test('UN knowledge-source', async ({ page }) => {
		// docs/tutorials/admin/02-knowledge-source.md
		// TODO: see /journeydoc-add-story
		await shoot(page, 'admin', '02-knowledge-source.png')
	})
